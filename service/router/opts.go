...
import (
	...
	"github.com/hardeepnarang10/collector/service/handler/v1"
	"github.com/hardeepnarang10/collector/service/healthcheck"
	"github.com/hardeepnarang10/collector/service/middleware/v1"
...
func WithHandler(svc *registry.ServiceRegistry, h handler.Handler) opts {
	if h == nil && svc == nil {
	...
	return func(r *router) error {
		collectSlice := []func(*fiber.Ctx) error{}
		if svc.ServiceConfig.EnableValidation {
			collectSlice = append(collectSlice, timeout.NewWithContext(mw.Collect, svc.ServiceConfig.RequestTimeout, errors.New("validation timed out")))
		}
		collectSlice = append(collectSlice, timeout.NewWithContext(h.Collect, svc.ServiceConfig.RequestTimeout, errors.New("handler timed out")))
		r.app.Post("/", collectSlice...).Name("Collect")
